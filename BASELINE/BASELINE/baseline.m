...
EDTstore = zeros(length(sampleSizeVec),length(sampleIntervalVec));
EDRstore = zeros(length(sampleSizeVec),length(sampleIntervalVec));
...
for i=1:length(sampleSizeVec) % Iterate over sample sizes
    ...
    for j=1:length(sampleIntervalVec) % Iterate over sample intervals
        ...
        [sampleData, EDP, EDT, EDR, perc95] = runWindowSampling_1(simData,numRuns,P,0,364,sampleSize,sampleInterval,tFinal,progress);
        EDPstore(i,j) = EDP;     
        EDTstore(i,j) = EDT; 
        EDRstore(i,j) = EDR; 
    end
