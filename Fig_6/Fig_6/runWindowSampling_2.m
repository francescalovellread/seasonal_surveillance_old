function [sampleData, EDP, EDT, EDR, ECDP, ESDP, perc95] = runWindowSampling_2(simData,numRuns,popSize,numSentinels,openWindow,closeWindow,cropSampleSize,sentinelSampleSize,sampleInterval,tFinal,progress)
...
%                 if (90<=mod(sampleTime,365) && mod(sampleTime,365)<273)
%                     theta = theta_S;
%                 else
%                     theta = theta_W;
%                 end
                ...
                cropStateVec = rand(1,numDetectableCrops); 
                cropStateVec(cropStateVec<1-theta)=0;
                cropStateVec(cropStateVec>=1-theta)=1;
                cropStateVec(Pc) = 0; % Create vector containing a 1 for every 'Detectable' crop plant and a 0 for every 'Undetectable' crop plant
                
                sentinelStateVec = rand(1,numDetectableSentinels); 
                sentinelStateVec(sentinelStateVec<1-theta)=0;
                sentinelStateVec(sentinelStateVec>=1-theta)=1;
                sentinelStateVec(Ps) = 0; % Create vector containing a 1 for every 'Detectable' sentinel plant and a 0 for every 'Undetectable' sentinel plant
